...
const features = [["⌘", "개발자만의 리뷰 공간", "GitHub 소셜 로그인으로 인증한 개발자만 리뷰를 남깁니다. 스팸 없이 신뢰할 수 있는 리뷰가 쌓입니다."], ["▣", "리뷰 = 독서 이력", "리뷰를 남기는 순간 나의 독서 이력에 자동으로 추가됩니다. 기록이 곧 성장의 증거가 됩니다."], ["↗", "독서 통계 대시보드", "월별 독서량, 카테고리 분포, 연속 독서 주간까지. 나의 독서 습관을 한눈에 확인하세요."], ["</>", "README.md 위젯", "프로필 README에 한 줄만 붙여넣으면, 실시간 독서 통계 위젯이 완성됩니다."], ["◎", "함께 나누는 리뷰", "동료 개발자들의 리뷰를 탐색하고, 다음에 읽을 책에 대한 새로운 인사이트를 발견하세요."], ["#", "마크다운 네이티브", "코드 블록, 인용, 링크까지. 개발자에게 익숙한 마크다운으로 자연스럽게 리뷰를 작성합니다."]];
...
export default function ReadthemPage() {
  const [isLaunchNoticeOpen, setIsLaunchNoticeOpen] = useState(false);

  const showLaunchNotice = () => setIsLaunchNoticeOpen(true);

  return <main>
    <header className="header"><Link className="logo" href="/" aria-label="ALLDAY PROJECT" /><nav><a href="#features">기능</a><a href="#how">작동 방식</a><a href="#widget">위젯</a></nav><button className="button small" type="button" onClick={showLaunchNotice}><GithubIcon /> <span>GitHub으로 시작</span></button></header>
    <section className="hero wrap" id="top"><div className="hero-copy"><p className="pill"><i /> 개발자 전용 · GitHub OAuth</p><h1>리뷰가 곧 <strong>독서 이력</strong>이 됩니다</h1><p>READTHEM.md는 개발자를 위한 도서 리뷰 플랫폼입니다. 읽은 책의 리뷰를 남기면 자동으로 독서 이력에 쌓이고, 그 통계를 GitHub 프로필 README.md 위젯으로 공유할 수 있어요.</p><div className="actions"><button className="button" type="button" onClick={showLaunchNotice}><GithubIcon /> GitHub으로 시작하기</button><a className="button ghost" href="#widget">위젯 미리보기 <b>→</b></a></div><code>{"// 리뷰 작성은 개발자 계정만 가능합니다"}</code></div><div className="readme-card"><div className="windowbar"><i /><i /><i /><span>octocat/octocat/README.md</span></div><div className="readme"><p>### 📚 My Reading</p><Heatmap /><div className="badges"><span>올해 42권</span><span>리뷰 128개</span><span>연속 12주</span></div></div></div></section>
    <section className="section" id="features"><div className="wrap"><div className="heading"><code>{"// features"}</code><h2>읽고, 기록하고, 공유하는 하나의 흐름</h2><p>리뷰 작성부터 통계 공유까지, 개발자의 독서 경험에 꼭 맞게 설계했습니다.</p></div><div className="feature-grid">{features.map(([icon, title, description]) => <article key={title}><b className="feature-icon">{icon}</b><h3>{title}</h3><p>{description}</p></article>)}</div></div></section>
    ...
    <section className="section ruled" id="widget"><div className="wrap"><div className="heading"><code>{"// readme widget"}</code><h2>복사, 붙여넣기. 그게 전부입니다</h2><p>아래 한 줄을 프로필 스페셜 레포지토리 README.md에 넣으면 위젯이 렌더링됩니다.</p></div><div className="widget-grid"><div className="code-box"><div className="card-title"><span>README.md</span><em>markdown</em></div><pre><span className="muted">&lt;!-- READTHEM.md 위젯 --&gt;</span>{"\n"}<b>![My Reading]</b>(https://readthem.md/<i>octocat</i>/widget.svg)</pre></div><div className="widget-card"><div className="widget-title"><b>&lt;/&gt;</b><span>octocat<span>&apos;s reading</span></span></div><div className="stat-grid">{[["42", "올해 읽은 책"], ["128", "작성한 리뷰"], ["12주", "연속 독서"], ["Backend", "즐겨 읽는 분야"]].map(([value, label]) => <div key={label}><b>{value}</b><span>{label}</span></div>)}</div><Heatmap /></div></div></div></section>
    <section className="section ruled"><div className="wrap cta"><div><code>{"// your next commit"}</code><h2>당신의 다음 커밋은 <strong>한 권의 책</strong>입니다</h2><p>지금 GitHub 계정으로 시작하고, 첫 리뷰를 독서 이력에 새겨보세요. 그리고 프로필에 자랑하세요.</p><button className="button" id="start" type="button" onClick={showLaunchNotice}><GithubIcon /> GitHub으로 무료 시작</button></div></div></section>
    <SiteFooter />
    {isLaunchNoticeOpen && <div className="fixed right-5 bottom-5 z-50 flex items-center gap-5 border border-[#111] bg-white px-5 py-4 text-sm font-bold shadow-[5px_5px_0_#111]" role="status"><span>READTHEM.md는 8/3에 출시됩니다.</span><button className="text-lg leading-none text-[#666] hover:text-[#111]" type="button" onClick={() => setIsLaunchNoticeOpen(false)} aria-label="알림 닫기">×</button></div>}
  </main>;
}
